...
export default function AuthProvider() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetResumeQuery(refreshToken);
  React.useEffect(() => {
    if (resumeData) {
      ...
      dispatch(
        setSalary({
          currency: resumeData.expected_salary_currency,
          salary: resumeData.expected_salary,
        }),
      );
  ...
  const tempRefreshToken = useSelector(
  ...
  return !areTokensFetched ? null : refreshToken &&
    !signUpCompleted &&
    signUpStep != 0 ? (
    <SignUp navigate={(page: string) => {}} screen={signUpStep} />
