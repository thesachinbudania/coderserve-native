...
export default function FollowingList() {
  ...
  const [nextPage, setNextPage] = React.useState<string | null>(`${apiUrl}/api/accounts/blocked_list/`);
...

      {
        filteredData.length === 0 && !isLoading && !initialLoading ? (
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 16, marginTop: -57 }}>
            ...
            <Text style={{ color: '#a6a6a6', textAlign: 'center', fontSize: 11 }}>{query ? 'No users match your search.' : "It looks like you haven't blocked anyone yet. Once you block a user, they'll appear here for easy management."}</Text>
          </View>
        ) :
          ...
          <FlatList
            ...
            onRefresh={() => {
              setRefreshing(true);
              refetch(`${apiUrl}/api/accounts/blocked_list/`);
            }}
