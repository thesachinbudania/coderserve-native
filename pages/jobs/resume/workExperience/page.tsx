...
import { Animated, Dimensions, Image, StyleSheet, Text, useAnimatedValue, View } from 'react-native';
...
export default function WorkExperience() {
	...
	async function searchCompanyName(name: string) {
		try {
			const response = await companiesData({ detail: false, data: name }).unwrap();
			if (response) {
	...
	console.log(companiesSuggestions)

	return (
		<>
			<Animated.View style={{ zIndex: 100, opacity: 0 }}>
			...
			>
				<Animated.View style={{ marginHorizontal: -16, alignItems: 'center' }}>
					<Animated.View style={[styles.formContainer, jRFocused || jDFocused ? { width: windowWidth } : { width: windowWidth - 32 }]}>
						...
						<Animated.View style={{ opacity: jDFocused ? 1 : contentOpacity }}>
							...
							>
								...
								/>
								<View style={{ display: jDFocused ? 'flex' : 'none', height: '100%', backgroundColor: '#f7f7f7' }}>
						...
						<Animated.View style={{ opacity: jRFocused ? 1 : contentOpacity }}>
							...
							>
								...
								/>
								<View style={{ display: jRFocused ? 'flex' : 'none', height: '100%', backgroundColor: '#f7f7f7', paddingTop: 8 }}>
									{companiesSuggestions.map((item, index) => (
										<View key={index} style={{ padding: 16, flexDirection: 'row', gap: 8 }}>
											<Image source={require('../../../profile/components/assets/searchIcon.png')} style={{ width: 20, height: 20 }} />
											<Text style={{ fontSize: 15 }}>{item.company_name}</Text>
										</View>
									))}
								</View>
							</Animated.View>
						</Animated.View>
						<Animated.View style={{ flexDirection: 'row', gap: 16, opacity: contentOpacity }}>
							<View style={{ flex: 1 / 2 }}>
						...
						<Animated.View style={{ opacity: contentOpacity, marginBottom: 48 }}>
							<BlueButton
								title='Save'
							/>
						</Animated.View>
