...
import { useDeleteCertificationMutation, useUpdateCertificationMutation, useAddCertificationMutation } from '../../apiSlice';
...
export default function OtherCertifications({ route }: { route: any }) {
	...
	const currentCertifications = useSelector((state: RootState) => state.jobs.otherCertifications);
	if (route.params && route.params.edit) {
		const certificationId = route.params.id;
		certification = currentCertifications?.find((certification) => certification.id === certificationId);
	}

	// final values for the form
	const [title, setTitle] = React.useState(certification ? certification.title : '');
	const [company, setCompany] = React.useState(certification ? certification.company : '');
	const [link, setLink] = React.useState(certification ? certification.link : '');

	const [error, setError] = React.useState('');
	const [popUpVisible, setPopUpVisible] = React.useState(false);


	const [addCertification, { isLoading: isAddCertificationLoading }] = useAddCertificationMutation();
	const [updateCertification, { isLoading: isUpdateCertificationLoading }] = useUpdateCertificationMutation();
	const [deleteCertification, { isLoading: isDeleteCertificationLoading }] = useDeleteCertificationMutation();
	...
	async function handleAddCertification() {
		...
		try {
			const response = await addCertification({ new_certification: data }).unwrap();
			if (response) {
	...
	const handleDeleteCertification = async () => {
		try {
			const response = await deleteCertification({ id: certification?.id }).unwrap();
			if (response) {
				dispatch(setOtherCertifications(response['other_certifications']));
			}
			navigation.goBack();
	...
	// value for searches
	const [titleText, setTitleText] = React.useState(certification ? certification.title : '');
	const [companyText, setCompanyText] = React.useState(certification ? certification.company : '');
	...
	return (
		...
		>
			...
			<Animated.View style={{ alignItems: 'center' }}>
				<View style={{ gap: 48, width: titleFocused || companyFocused ? windowWidth : windowWidth - 32 }}>
					...
					<Animated.View style={{ opacity: companyFocused ? 1 : contentOpacity }}>
						<Animated.View style={{ opacity: contentOpacity }}>
					...
					<View style={{ gap: 16 }}>
						<BlueButton
							...
							onPress={certification ? handleUpdateCertification : handleAddCertification}
							loading={certification ? isUpdateCertificationLoading : isAddCertificationLoading}
							disabled={certification ? (certification.title === title && certification.link === link && certification.company === company) || !title || !company || !link : !title || !company || !link}
						/>
						{
							certification && <NoBgButton
								title='Delete'
