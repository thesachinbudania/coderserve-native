...
import { useDeleteLanguagesMutation, useUpdateLanguagesMutation, useAddLanguagesMutation } from '../../apiSlice';
...
export default function Languages({ route }: { route: any }) {
	...
	const [language, setLanguage] = React.useState<string | null>(currentLanguage ? currentLanguage.language : null);
	const [rating, setRating] = React.useState<number>(currentLanguage ? currentLanguage.rating : 0);
	...
	const [addLanguage, { isLoading: isAddLanguageLoading }] = useAddLanguagesMutation();
	const [updateLanguage, { isLoading: isUpdateLanguageLoading }] = useUpdateLanguagesMutation();
	const [deleteLanguage, { isLoading: isDeleteLanguageLoading }] = useDeleteLanguagesMutation();
	...
	const handleDeleteLanguage = async () => {
		try {
			const response = await deleteLanguage({ id: currentLanguage?.id }).unwrap();
			if (response) {
				dispatch(setLanguages(response['languages']));
			}
			navigation.goBack();
	...
	return (
		...
		>
			...
			<View style={languageStyles.ratingContainer}>
				{
					rating > 0 && (
						<Text style={languageStyles.ratingWord}>{ratingWords[rating - 1]}</Text>
					)
				}
				<Rating
					stars={rating}
...
export function Rating({ size = 48, stars, editable = true, setStars = () => { } }: { size?: number, stars: number, editable?: boolean, setStars?: React.Dispatch<React.SetStateAction<number>> }) {
	const maxRating = 5;
				...
				Array.from({ length: maxRating }, (_, index) => (
					...
					>
						<View key={index} style={languageStyles.starContainer}>
							<Image source={index < stars ? selectedStar : unselectedStar} style={{ height: size, width: size }} />
						</View>
					</Pressable>
				))
			}
